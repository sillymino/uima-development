...
public class EntityAnnotatorTest extends TestCase {
	...
	private static final String DESCRIPTOR_TYPESYSTEM = "src/test/resources/tsDescriptor.xml";
	private static final String XMI_FILE = "src/test/resources/abbrtest.xmi";
	private static final String ENTITY_ANNOTATOR_DESC = "src/test/resources/EntityAnnotatorTest.xml";
	private static final String NEGATIVE_LIST = "src/test/resources/negativeList";
	...
	/**
	 * test whether process method runs successfully. Output must be checked by a human manually
	 */
	/*public void testProcess() throws InvalidXMLException, ResourceInitializationException, IOException, SAXException,
		...
		AnalysisEngine entityAnnotator = null;
		try {
			XMLInputSource taggerXML = new XMLInputSource(ENTITY_ANNOTATOR_DESC);
		...
		CAS myCAS = getCasFromXMI(XMI_FILE);
		
		entityAnnotator.process(myCAS);
	}*/

	/**
	 * This function tests parts of the process method, including building the unit sentence and
	 * removing duplicates. Prediction is "simulated" (labels are set).
	 */
	/*public void testSimulatedProcess() throws IllegalAccessException, NoSuchFieldException,
		...
		// expected result
		int[] expandedSentLength = new int[] { 11, 12, 5 };
		int[] originalSentLength = new int[] { 11, 7, 5 };

		// make a new annotator
		EntityAnnotator annotator = new EntityAnnotator();
		annotator.expandAbbr = true;
		annotator.showSegmentConf = false;
		annotator.consistPreservation = false;
		
		annotator.sentence_type = "de.julielab.jules.types.Sentence";
		annotator.token_type = "de.julielab.jules.types.Token";
		annotator.pos_type = "de.julielab.jules.types.POSTag";

		// load old XMI
		LOGGER.debug("testSimulatedProcess() - getting CAS");
		CAS myCAS = getCasFromXMI(XMI_FILE);
		JCas aJCas = myCAS.getJCas();

		// iterate over sentences
		LOGGER.debug("testSimulatedProcess() - getting data from CAS");
		JFSIndexRepository indexes = myCAS.getJCas().getJFSIndexRepository();
		...
		while (sentenceIter.hasNext()) {
			...
			ArrayList<Annotation> tokenList = (ArrayList<Annotation>) UIMAUtils.getAnnotations(aJCas, sentence, (new Annotation(aJCas, 0, 0)).getClass());
			// ArrayList<Abbreviation> abbrevList = annotator.getAbbreviationList(tokenList, aJCas);
			ArrayList<HashMap<String, String>> metaList = new ArrayList<HashMap<String, String>>();
			for (Annotation token : tokenList) { // empty metas for testing here
	...
	/**
	 * Test whether adding the predicted labels as entities to CAS works correctly. Testing correct
	 * handling of negative list is included, thus two entity mentions found should not be added to
	 * CAS ("NN" with label "A", and "UU" with any label).
	 * 
	...
	public void testWriteToCAS() throws SecurityException, NoSuchFieldException, ResourceInitializationException,
		...
		Field negativeList = entityAnnotator.getClass().getDeclaredField("negativeList");
		negativeList.setAccessible(true);
		negativeList.set(entityAnnotator, new NegativeList(new File(NEGATIVE_LIST)));

		// this HashMap is a global variable in EntityAnnotator and is normally initialized
		// within the initialize() method. In this test cast we have to build our own.
		HashMap<String, String> entityHashMap = new HashMap<String, String>();
		...
		// use reflection to get private field
		Field entityMap = entityAnnotator.getClass().getDeclaredField("entityMap");
		...
		unitSentence.add(new Unit(8, 9, "N", "A")); // should be ignored due to negative list
		unitSentence.add(new Unit(9, 10, "N", "A"));// should be ignored due to negative list
		unitSentence.add(new Unit(10, 11, "O", "O"));
		unitSentence.add(new Unit(11, 12, "U", "A"));// should be ignored due to negative list
		unitSentence.add(new Unit(12, 13, "U", "A"));// should be ignored due to negative list
		...
		while (entityIter.hasNext()) {
			...
			//LOGGER.debug("testWriteToCAS() - covered Text: " + entity.getCoveredText() + " -> specific type: " + entity.getSpecificType());
			LOGGER.debug("testWriteToCAS() - covered Text: " + entity.getCoveredText());

			String trueLabel = result.get(entity.getCoveredText());
	 ...
	 */
	private CAS getCasFromXMI(String fileName) throws InvalidXMLException, ResourceInitializationException,
					IOException, SAXException {
		CAS cas = null;

		LOGGER.debug("reading from: " + DESCRIPTOR_TYPESYSTEM);
		TypeSystemDescription tsDesc = getTypeSystemDescription(DESCRIPTOR_TYPESYSTEM);
	 ...
	 */
	public TypeSystemDescription getTypeSystemDescription(String tsDescriptorName) throws InvalidXMLException,
