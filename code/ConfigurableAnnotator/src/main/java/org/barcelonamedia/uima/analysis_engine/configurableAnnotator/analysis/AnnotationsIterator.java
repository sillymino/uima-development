...
import org.barcelonamedia.uima.analysis_engine.configurableAnnotator.mapping.AnnotationDescription;
import org.barcelonamedia.uima.analysis_engine.configurableAnnotator.mapping.AnnotationsDescription;
import org.barcelonamedia.uima.analysis_engine.configurableAnnotator.mapping.FeatureDescription;
import org.barcelonamedia.uima.analysis_engine.configurableAnnotator.mapping.FeaturesDescription;
...
public class AnnotationsIterator {
     ...
     *            extracted.
     */
    public AnnotationsIterator(final JCas cas, final AnnotationsDescription annotationsDesc, AnnotationFS coveringAnnotation){

        this.jCas = cas;
        this.annotationDescriptions = annotationsDesc.getAnnotationDescriptions();
     ...
     *             defined.
     */
    public final String next() throws InvalidFeatureException, InexistentAnnotationException{
        ...
        while(featureValuesIterator == null || !featureValuesIterator.hasNext()){
            ...
            featureValueList = getFeatureValuesList(currentAnnotation, currentAnnotDesc);
    ...
    private void setAnnotationIterator() throws InexistentAnnotationException{
        ...
        Type annotationType = this.jCas.getTypeSystem().getType(currentAnnotDesc.getType());
        
        AnnotationFS coveredAnnotation = this.jCas.getCas().createAnnotation(annotationType, 0, 0);
        
        List<Annotation> coveredAnnotations = JCasUtil.selectCovered(this.jCas, (Class<Annotation>)coveredAnnotation.getClass(), this.coveringAnnotation);
        Iterator<Annotation> it = coveredAnnotations.iterator();
        
        annotationIterator = Iterators.filter(it, new NotNullPredicate<Annotation>());
        
        if(!annotationIterator.hasNext()){
        	
            throw new InexistentAnnotationException("The annotation type '" + currentAnnotDesc.getType() + "' does not exists in this document.");
        }
    }
